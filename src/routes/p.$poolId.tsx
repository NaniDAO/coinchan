...
} from "@/components/pools/PredictionMarketDisplay";
...
function PoolPage() {
    ...
    const { data: cookbookPool, isLoading: cookbookLoading } = useGetPool(
        poolId,
        "COOKBOOK",
    );
    const { data: zammPool, isLoading: zammLoading } = useGetPool(
        poolId,
        "ZAMM",
    );
    ...
    const isPredictionMarket =
        pammData?.isPAMMPool && pammData?.marketId !== null;
            ...
            <div className="container mx-auto px-4 py-8 max-w-4xl">
                ...
                <div className="text-center py-20">
                    ...
                    <p className="text-muted-foreground">
                        Pool #{poolId} does not exist or has not been indexed
                        yet.
                    </p>
                </div>
            </div>
                ...
                <div className="mt-6 flex gap-3 flex-wrap">
                    ...
                    <Link
                        ...
                        search={
                            {
                                tokenA:
                                    pool.token0 && pool.coin0?.id
                                        ? `${pool.token0}:${pool.coin0.id}`
                                        : undefined,
                                tokenB:
                                    pool.token1 && pool.coin1?.id
                                        ? `${pool.token1}:${pool.coin1.id}`
                                        : undefined,
...
function PredictionMarketPoolInfo({
    ...
    const reserve0 = pool.reserve0
        ? Number(formatEther(BigInt(pool.reserve0)))
        : null;
    const reserve1 = pool.reserve1
        ? Number(formatEther(BigInt(pool.reserve1)))
        : null;
    ...
    return (
        <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
            <InfoCard
                ...
                value={
                    pammData.rYes !== null
                        ? formatNumber(Number(formatEther(pammData.rYes)))
                        : "—"
                }
            ...
            <InfoCard
                ...
                value={
                    pammData.rNo !== null
                        ? formatNumber(Number(formatEther(pammData.rNo)))
                        : "—"
                }
            ...
            <InfoCard
                label="YES Supply"
                value={
                    pammData.yesSupply !== null
                        ? formatCollateral(pammData.yesSupply)
                        : "—"
                }
            />
            <InfoCard
                label="NO Supply"
                value={
                    pammData.noSupply !== null
                        ? formatCollateral(pammData.noSupply)
                        : "—"
                }
            />
            <InfoCard
...
function PoolHeader({
    ...
    const priceEth = pool.price1
        ? Number(formatEther(BigInt(pool.price1)))
        : null;
...
function PoolInfo({ pool, ethUsdPrice }: { pool: Pool; ethUsdPrice?: number }) {
    const reserve0Eth = pool.reserve0
        ? Number(formatEther(BigInt(pool.reserve0)))
        : null;
    const reserve1Eth = pool.reserve1
        ? Number(formatEther(BigInt(pool.reserve1)))
        : null;

    const totalLiquidityEth = (reserve0Eth || 0) + (reserve1Eth || 0);
    const totalLiquidityUsd = ethUsdPrice
        ? totalLiquidityEth * ethUsdPrice
        : null;
    ...
    return (
        <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <InfoCard
                label="Token 0"
                value={pool.coin0?.symbol || "—"}
                subValue={shortAddr(pool.token0)}
            />
            <InfoCard
                label="Token 1"
                value={pool.coin1?.symbol || "—"}
                subValue={shortAddr(pool.token1)}
            />
            <InfoCard
                label="Reserve 0"
                value={reserve0Eth != null ? formatNumber(reserve0Eth) : "—"}
            />
            <InfoCard
                label="Reserve 1"
                value={reserve1Eth != null ? formatNumber(reserve1Eth) : "—"}
            />
        </div>
    );
}
