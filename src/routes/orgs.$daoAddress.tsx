...
export const Route = createFileRoute("/orgs/$daoAddress")({
  component: OrgPage,
});
...
const fromEpoch = (s: number) =>
  new Date(s * 1000).toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" });
const shortenAddress = (addr: string) => `${addr.slice(0, 6)}...${addr.slice(-4)}`;

function OrgPage() {
  const { daoAddress } = Route.useParams();
  const { data: org, isLoading, error } = useDAICOOrg(daoAddress);
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2 text-sm text-muted-foreground mt-1">
            ...
            <span>•</span>
            <a
              href={`https://etherscan.io/address/${org.id}`}
              target="_blank"
            ...
            <Card key={purchase.id} className="p-4">
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <a
                      href={`https://etherscan.io/address/${purchase.buyer}`}
                      target="_blank"
                      rel="noopener noreferrer"
            ...
            <Card className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <a
                    href={`https://etherscan.io/address/${org.tap.ops}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="font-medium hover:underline inline-flex items-center gap-1"
                  >
            ...
            {org.lps.map((lp, idx) => (
              <Card key={idx} className="p-6">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <a
                    href={`https://etherscan.io/tx/${lp.txHash}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-sm text-muted-foreground hover:text-foreground transition-colors inline-flex items-center gap-1"
                  >
