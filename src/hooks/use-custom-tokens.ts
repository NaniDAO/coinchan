...
export function useCustomTokens() {
  ...
  const addCustomToken = useCallback(
    async (address: Address): Promise<TokenMeta | null> => {
      ...
      try {
        ...
        const existing = customTokens.find((t) => t.token1?.toLowerCase() === checksummedAddress.toLowerCase());
        if (existing) {
        ...
        const name = nameResult.status === "success" ? (nameResult.result as string) : symbol;
  ...
  const removeCustomToken = useCallback(
    (address: Address) => {
      const checksummedAddress = getAddress(address);
      const updatedTokens = customTokens.filter((t) => t.token1?.toLowerCase() !== checksummedAddress.toLowerCase());
      setCustomTokens(updatedTokens);
  ...
  const isCustomToken = useCallback(
    (address?: string): boolean => {
      ...
      try {
        ...
        return customTokens.some((t) => t.token1?.toLowerCase() === checksummedAddress.toLowerCase());
      } catch {
        return false;
      }
    },
