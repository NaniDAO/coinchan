...
const fetchCoinData = async (coinId: string, token: Address, chainId: number = 1) => {
  try {
    const response = await fetch(import.meta.env.VITE_INDEXER_URL + "/graphql", {
      ...
      body: JSON.stringify({
        query: `
            query GetCoinData {
              coin(id: "${coinId.toString()}", token: "${token.toString()}", chainId: ${chainId}) {
                id
                name
                symbol
                description
...
export const useGetCoin = ({
  ...
  return useQuery<GetCoinData>({
    ...
    queryFn: () => fetchCoinData(coinId, token, chainId),
  });
};
