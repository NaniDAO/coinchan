...
/**
 * Uses viem `multicall` to ask QuoterV2 for all candidate paths in one RPC.
 * Returns best amountOut (EXACT_IN) or best amountIn (EXACT_OUT).
 */
function useUniV3BestQuote({
  ...
  const { data, isFetching, isError, error } = useQuery({
    ...
    enabled,
    staleTime: 5_000, // quotes get stale fast, but cache briefly to dedupe
    gcTime: 60_000,
    ...
    queryFn: async () => {
      ...
      const calls = [...direct, ...viaWeth].map((c) => ({
      ...
      // Scan results for best value
      if (side === "EXACT_IN") {
        let bestOut = 0n;
        for (const r of res) {
          if (r.status === "success") {
            const out = (r.result as any)[0] as bigint; // amountOut
            if (out > bestOut) bestOut = out;
          }
        }
        return { uniAmount: bestOut };
      } else {
        let bestIn: bigint | null = null;
        for (const r of res) {
          if (r.status === "success") {
            const ain = (r.result as any)[0] as bigint; // amountIn
            if (ain > 0n && (bestIn === null || ain < bestIn)) bestIn = ain;
          }
        }
        return { uniAmount: bestIn ?? 0n };
      }
    },
  });
...
export function useZRouterVsUniEfficiency(opts: {
  ...
  // zRouter quote (keep your existing hook; just allow it to debounce via params)
  ...
  // Uni best quote (single multicall)
