...
const resolveVisual = (
  ...
  if (symbol && specialLogos[symbol])
    return { kind: "img", src: specialLogos[symbol] };
...
export const PoolTokenImage = memo(
  ({
    ...
    const left = useMemo(
      () => resolveVisual(symbol0, imageUrl0),
      [symbol0, imageUrl0],
    );
    const right = useMemo(
      () => resolveVisual(symbol1, imageUrl1),
      [symbol1, imageUrl1],
    );
    ...
    const LeftHalf = () => {
      ...
      return (
        <div
          className={cn(
            "w-1/2 h-full flex items-center justify-center border-r-2 border-muted text-[10px]",
            bg,
            text,
          )}
        >
    ...
    const RightHalf = () => {
      ...
      return (
        <div
          className={cn(
            "w-1/2 h-full flex items-center justify-center text-[10px]",
            bg,
            text,
          )}
        >
    ...
    return (
      <div
        className={cn(
          "relative w-8 h-8 rounded-full overflow-hidden flex border border-border",
          className,
        )}
      >
        <LeftHalf />
