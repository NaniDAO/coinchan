...
export const PoolTokenImage = memo(({ imageUrl0, imageUrl1, className }: PoolTokenImageProps) => {
  ...
  return (
    <div className={cn("relative w-8 h-8 rounded-full overflow-hidden flex border border-border", className)}>
      ...
      {imageUrl0 && !error0 ? (
        ...
        <div className="w-1/2 h-full flex items-center justify-center bg-gray-700 text-white text-[10px]">T0</div>
      )}
      ...
      {imageUrl1 && !error1 ? (
        ...
        <div className="w-1/2 h-full flex items-center justify-center bg-gray-700 text-white text-[10px]">T1</div>
      )}
    </div>
  );
});
