...
function FinalizedPoolTradingInner({
    ...
    const actualTotalSupply = useMemo(() => {
        ...
        if (totalSupply && totalSupply > 0n) {
            console.log("Using totalSupply:", totalSupply);
            return totalSupply;
        }

        // Try to get it from coinData (but check it's not 0)
        if (coinData?.totalSupply && coinData.totalSupply > 0n) {
            console.log("Using COINDATA totalSupply:", coinData?.totalSupply);
        ...
        if (holdersTotalSupply && holdersTotalSupply > 0n) {
            console.log("Using Holder totalSupply:", holdersTotalSupply);
    ...
    const { coinPrice, coinUsdPrice, marketCapUsd, marketCapEth } =
        useMemo(() => {
            if (
                ...
                console.warn(
                    "No reserves available for pool:",
                    poolId,
                    reserves,
                );
                return {
    ...
    return (
        <div>
            ...
            <div className="flex flex-col lg:grid lg:grid-cols-10 gap-4">
                ...
                >
                    ...
                    <TabsContent value="swap" className="mt-4">
                        <ErrorBoundary
                            fallback={<div>Error loading LockedSwapTile</div>}
                        >
            ...
            <ErrorBoundary
                fallback={
                    <ErrorFallback errorMessage="Error loading Pool Info" />
                }
            >
                {/* Info Section */}
                <PoolInfoSection
