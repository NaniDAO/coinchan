    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between border-b border-border pb-3">
        ...
        <a
          ...
          className="px-4 py-2 border border-border hover:border-border/60 bg-card hover:bg-accent font-mono text-sm tracking-wider transition-all"
        >
          GOVERN →
        </a>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="p-4 border border-border rounded-lg bg-card">
          <div className="text-xs text-muted-foreground mb-1">Quorum Requirement</div>
          <div className="text-2xl font-bold font-mono">{quorumBps ? (quorumBps / 100).toFixed(1) : 0}%</div>
          <div className="text-xs text-muted-foreground mt-2">Minimum votes needed for proposals to pass</div>
        </div>

        {/* Proposal Threshold */}
        <div className="p-4 border border-border rounded-lg bg-card">
          <div className="text-xs text-muted-foreground mb-1">Proposal Threshold</div>
          <div className="text-2xl font-bold font-mono">
            {proposalThreshold ? formatUnits(proposalThreshold, 18) : "0"}
          </div>
          <div className="text-xs text-muted-foreground mt-2">Voting power needed to create proposals</div>
        </div>

        {/* Proposal TTL */}
        <div className="p-4 border border-border rounded-lg bg-card">
          <div className="text-xs text-muted-foreground mb-1">Proposal Duration (TTL)</div>
          <div className="text-2xl font-bold font-mono">{proposalTTL ? formatDuration(proposalTTL) : "N/A"}</div>
          <div className="text-xs text-muted-foreground mt-2">How long proposals remain active for voting</div>
        </div>

        {/* Timelock Delay */}
        <div className="p-4 border border-border rounded-lg bg-card">
          <div className="text-xs text-muted-foreground mb-1">Timelock Delay</div>
          <div className="text-2xl font-bold font-mono">{timelockDelay ? formatDuration(timelockDelay) : "N/A"}</div>
          <div className="text-xs text-muted-foreground mt-2">Delay after proposal succeeds before execution</div>
        </div>

        {/* Ragequittable */}
        <div className="p-4 border border-border rounded-lg bg-card">
          <div className="text-xs text-muted-foreground mb-1">Ragequit Status</div>
        ...
        {/* Auto-Futarchy */}
        <div className="p-4 border border-border rounded-lg bg-card">
          <div className="text-xs text-muted-foreground mb-1">Auto-Futarchy</div>
          <div className="text-2xl font-bold font-mono">
