...
export function DAICOTradeCard({
    daoAddress,
    sale,
    chainId,
}: DAICOTradeCardProps) {
    ...
    const price = useMemo(() => {
        ...
        return (
            parseFloat(formatEther(BigInt(sale.tribAmt))) /
            parseFloat(formatEther(BigInt(sale.forAmt)))
        );
    }, [sale]);
    ...
    const progress = useMemo(() => {
        ...
        const calculated = (
            (parseFloat(formatEther(BigInt(sale.totalRaised))) /
                parseFloat(formatEther(BigInt(sale.forAmt)))) *
            100
        );
    ...
    const handleBuy = useCallback(async () => {
        ...
        try {
            ...
            const minBuyAmount =
                quotedAmount && quotedAmount > 0n
                    ? (quotedAmount * 95n) / 100n
                    : 0n; // 5% slippage
                ...
                <div className="flex flex-col items-center justify-center gap-3 text-center">
                    ...
                    <div>
                        <h3 className="font-semibold mb-1">
                            {!sale ? "No Active Sale" : "Sale Ended"}
                        </h3>
                        <p className="text-sm text-muted-foreground">
                            {!sale
                                ? "This DAO doesn't have an active token sale"
                                : "The token sale has ended or is no longer active"}
                        </p>
                    </div>
                </div>
            </Card>
            ...
            <div className="space-y-4 flex-1 flex flex-col">
                ...
                <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                        Purchase DAO tokens from the sale
                    </p>
                </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 p-3 rounded-lg bg-muted/30">
                        <div>
                            ...
                            <div className="font-mono text-sm">
                                {ethBalance
                                    ? `Ξ${parseFloat(formatEther(ethBalance.value)).toFixed(4)}`
                                    : "0.0000"}
                            </div>
                        </div>
                        <div>
