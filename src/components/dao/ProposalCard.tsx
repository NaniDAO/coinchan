...
export const ProposalCard = ({ proposalId }: ProposalCardProps) => {
    ...
    const totalVotes = tallies
        ? tallies.forVotes + tallies.againstVotes + tallies.abstainVotes
        : 0n;
    const forPercentage =
        totalVotes > 0n
            ? Number((tallies!.forVotes * 10000n) / totalVotes) / 100
            : 0;
    const againstPercentage =
        totalVotes > 0n
            ? Number((tallies!.againstVotes * 10000n) / totalVotes) / 100
            : 0;
                ...
                <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                        <h3 className="font-semibold text-sm font-mono truncate">
                            {t("dao.proposal")} #
                            {proposalId.toString().slice(0, 10)}...
                        </h3>
                        <span
                        ...
                        <div className="mt-2 space-y-2 mb-4">
                            <div className="flex justify-between text-xs">
                                <span className="text-green-400">
                                    {t("dao.for")}:{" "}
                                    {tallies.forVotes.toString()}
                                </span>
                                <span className="text-red-400">
                                    {t("dao.against")}:{" "}
                                    {tallies.againstVotes.toString()}
                                </span>
                            </div>
