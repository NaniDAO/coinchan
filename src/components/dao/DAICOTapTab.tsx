...
export function DAICOTapTab({ tap, explorerUrl, fmtETH, fromEpoch, shortenAddress }: DAICOTapTabProps) {
  return (
    <Card className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <div className="font-medium">{fmtETH(tap.totalClaimed)}</div>
        </div>
        <div>
        ...
        <div>
          ...
          <div className="font-medium">{fromEpoch(tap.lastClaim)}</div>
        </div>
      </div>
    </Card>
  );
