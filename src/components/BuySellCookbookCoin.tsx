...
const BuySellCookbookCoinInner = ({
    ...
    const [liquiditySubTab, setLiquiditySubTab] = useState<"dual" | "zap">(
        "dual",
    );
    ...
    const actualSwapFee = useMemo(() => {
        if (coinData?.pools && coinData.pools.length > 0) {
            ...
            const ethPool = coinData.pools.find(
                (pool: any) => pool.coin0Id === 0n,
            );
            if (ethPool?.swapFee) {
    ...
    const canClaim = useMemo(() => {
        ...
        const isFinalized =
            saleData &&
            saleData[0] === "0x0000000000000000000000000000000000000000";
        return (
            isFinalized &&
            launchpadBalance &&
        ...
        try {
            if (tab === "buy" && ethBalance) {
                ...
                if (ethBalance.value > 0n) {
                    ...
                    setPercentage(
                        Math.min(100, Math.max(0, calculatedPercentage)),
                    );
                }
        ...
        const timer = setTimeout(() => {
            try {
                ...
                const currentPriceInEth =
                    Number(currentPrice) / Number(scaleFactor);
                ...
                const impactPercent =
                    ((newPriceInEth - currentPriceInEth) / currentPriceInEth) *
                    100;
    ...
    const handlePercentageChange = (newPercentage: number) => {
        ...
        if (tab === "buy" && ethBalance) {
            ...
            const adjustedBalance =
                (coinBalance * BigInt(newPercentage)) / 100n;
    ...
    return (
        <div className="space-y-4">
                    ...
                    <div className="p-2 bg-muted/30 rounded-lg text-xs text-muted-foreground">
                        <div className="flex flex-col gap-1">
                            {(() => {
                                ...
                                const tokenPriceUsd =
                                    tokenPriceInEth * ethPrice.priceUSD;
                                const totalPoolValueUsd =
                                    ethAmount * ethPrice.priceUSD * 2;

                                return (
                                    <>
                                        <div className="opacity-90">
                                            Pool Value: $
                                            {formatNumber(totalPoolValueUsd, 2)}{" "}
                                            USD
                                        </div>
                                        <div className="opacity-75">
                                            1 ETH ={" "}
                                            {formatNumber(ethPriceInToken, 6)}{" "}
                                            {symbol} | 1 {symbol} ={" "}
                                            {tokenPriceInEth.toFixed(8)} ETH ($
                                            {formatNumber(
                                                tokenPriceUsd,
                                                8,
                                            )}{" "}
            ...
            <Tabs
                value={tab}
                onValueChange={(v) =>
                    setTab(v as "buy" | "sell" | "add" | "remove")
                }
            >
                ...
                <TabsContent value="buy">
                    <div className="flex flex-col gap-2">
                        ...
                        <div className="flex gap-2">
                            <Input
                                type="number"
                                placeholder={t("create.amount_token", {
                                    token: "ETH",
                                })}
                                value={amount}
                                onChange={(e) =>
                ...
                <TabsContent value="add">
                    <div className="space-y-4">
                        ...
                        <Tabs
                            value={liquiditySubTab}
                            onValueChange={(v) =>
                                setLiquiditySubTab(v as "dual" | "zap")
                            }
                        >
                ...
                {showPriceChart && (
                    <div className="transition-all duration-300 rounded-lg border border-border p-4 bg-card">
                        <div className="text-xs text-muted-foreground mb-2">
                            {symbol}/ETH{" "}
                            {t("coin.price_history", "Price History")}
                        </div>
                        <PoolPriceChart
