...
export const TokenPage = ({ token }: { token: Token }) => {
    ...
    const { data: reserves } = useReserves({
        poolId: poolId,
        source:
            coinData?.pools?.[0]?.protocol === "ZAMMV0" ? "ZAMM" : "COOKBOOK",
    });
        ...
        useMemo(() => {
            ...
            if (zcurveSale && zcurveSale.status === "ACTIVE") {
                ...
                if (netSold > 0n && ethEscrow > 0n) {
                    ...
                    const marketCapWei =
                        (totalSupply * effectivePriceWei) / 10n ** 18n;

                    if (marketCapWei > 0n) {
                        const impliedMarketCapEth = Number(
                            formatEther(marketCapWei),
                        );
                ...
                if (currentPriceWei > 0n) {
                    const marketCapWei =
                        (totalSupply * currentPriceWei) / 10n ** 18n;
                    if (marketCapWei > 0n) {
                        const impliedMarketCapEth = Number(
                            formatEther(marketCapWei),
                        );
            ...
            if (
                ...
                const marketCapUsd = marketCapEth
