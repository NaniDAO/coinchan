...
export const TokenPage = ({ token }: { token: Token }) => {
  ...
  // Extract coin data
  const [name, symbol, imageUrl, description, tokenURI, poolIds, swapFees, totalSupply, tokenMetadata] = useMemo(() => {
    if (!coinData)
      return [
        tokenData?.name,
        tokenData?.symbol,
  ...
  const poolId = useMemo(() => {
    ...
    return (
      ...
      computePoolId(
        tokenA,
        tokenB,
        BigInt(coinData?.pools?.[0]?.feeOrHook ?? coinData?.pools?.[0]?.swapFee ?? SWAP_FEE),
  ...
  const { data: holdersTotalSupply } = useCoinTotalSupply(token?.id?.toString(), reserves);
      ...
      <ErrorBoundary fallback={<ErrorFallback errorMessage="Error rendering Coin Info Card" />}>
        {tokenData ? (
          <CoinInfoCard
            ...
            name={name}
            symbol={symbol}
            description={description ?? tokenData?.description ?? t("coin.no_description", "No description available")}
            imageUrl={imageUrl}
