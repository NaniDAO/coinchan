...
export function ZCurveSaleProgress({ sale }: ZCurveSaleProgressProps) {
  ...
  const quadCapPercentage = saleCap > 0n ? Number((quadCap * 100n) / saleCap) : 0;
    ...
    <CardContent className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <div className="flex justify-between text-xs text-muted-foreground">
          ...
          <span>{t("sale.target", "Target")}: {formatEther(ethTarget)} ETH</span>
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="flex justify-between text-xs text-muted-foreground">
          <span>{formatEther(netSold).slice(0, 8)} {t("common.sold", "sold")}</span>
          <span>{formatEther(saleCap).slice(0, 8)} {t("common.cap", "cap")}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 pt-2">
        <div className="space-y-1">
          <p className="text-xs text-muted-foreground">{t("sale.current_price", "Current Price")}</p>
          <p className="text-sm font-medium">
            {sale.currentPrice ? formatEther(BigInt(sale.currentPrice)).slice(0, 10) : "0"} ETH
          </p>
        </div>
        <div className="space-y-1">
          <p className="text-xs text-muted-foreground">{t("sale.pricing_phase", "Pricing Phase")}</p>
          <p className="text-sm font-medium">
            {netSold < quadCap 
