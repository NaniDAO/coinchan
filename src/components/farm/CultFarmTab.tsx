...
export function CultFarmTab() {
  ...
  const cultFarms = useMemo(() => {
    ...
    return allStreams.filter(stream => 
      BigInt(stream.lpId) === CULT_POOL_ID && 
...
function CultFarmCard({ farm, lpBalance, onHarvest, isHarvesting }: CultFarmCardProps) {
  ...
  const isActive = timeRemaining.seconds > 0;
  const rewardTokenDecimals = farm.rewardCoin?.decimals || 18;
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <div className="bg-black/30 p-3 rounded border border-red-900/20 md:col-span-2">
            <APYDisplay stream={farm} lpToken={CULT_TOKEN} shortView={true} />
          </div>
        </div>
          ...
          <div className="bg-green-900/10 border border-green-600/30 p-4 rounded">
            <div className="grid grid-cols-2 gap-3 mb-3">
              ...
              <div>
                ...
                <p className="text-sm font-mono font-bold text-green-400">
                  {formatBalance(formatEther(pendingRewards || 0n), farm.rewardCoin?.symbol)}
                </p>
              </div>
            </div>
          </div>
