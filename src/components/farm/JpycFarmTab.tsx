        ...
        try {
            ...
            return allStreams.filter((stream) => {
                try {
                    ...
                    const matchesChefId =
                        BigInt(stream.chefId) === JPYC_FARM_CHEF_ID;
                    ...
                    console.error(
                        `Error processing stream ${stream?.chefId}:`,
                        err,
                    );
                    return false;
                }
            ...
            <div className="text-center py-12">
                <div className="bg-muted/10 border border-primary/30 rounded-lg p-8">
                    <p className="text-primary font-mono text-lg mb-2">
                        [ {t("common.error_loading_farms_label")} ]
                    </p>
                    <p className="text-muted-foreground text-sm">
                        {t("common.error_loading_farms")}
                    </p>
                </div>
            </div>
        );
    ...
    if (!userAddress) {
        return (
            <div className="text-center py-12">
                <div className="bg-muted/10 border border-primary/30 rounded-lg p-8">
                    <p className="text-primary font-mono text-lg mb-2">
                        [ {t("common.connect_wallet_label")} ]
                    </p>
                    <p className="text-muted-foreground text-sm">
                        {t("common.connect_wallet_to_view_farms")}
                    </p>
                </div>
            </div>
        );
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="text-sm text-muted-foreground font-mono">
                    {formatBalance(formatEther(lpBalance), "LP")}{" "}
                    {t("common.available")}
                </div>
            </div>
            ...
            {isLoadingStreams ? (
                <div className="space-y-4">
                    {[...Array(2)].map((_, i) => (
                        <div
                            key={i}
                            className="h-48 bg-muted/10 border border-primary/30 rounded-lg animate-pulse"
                        />
                    ))}
                </div>
            ) : jpycFarms.length === 0 ? (
                <div className="text-center py-12">
                    <div className="bg-muted/10 border border-primary/30 rounded-lg p-8">
                        <p className="text-primary font-mono text-lg mb-2">
                            [ {t("common.no_active_farms_label")} ]
                        </p>
                        <p className="text-muted-foreground text-sm">
                            {t("common.no_jpyc_farms_active")}
                        </p>
                    </div>
                </div>
            ) : (
                <div className="grid gap-4">
                    {jpycFarms.map((farm) => {
                        if (!farm || !farm.chefId) {
                            console.error(
                                "Invalid farm data in jpycFarms:",
                                farm,
                            );
                            return null;
                        }
...
function JpycFarmCard({
    ...
    const {
        ...
        tokenAddress: isErc20Reward
            ? (farm.rewardToken as `0x${string}`)
            : undefined,
    });
    ...
    const isDaiToken =
        farm.rewardToken?.toLowerCase() === DAI_ADDRESS.toLowerCase();
    ...
    const rewardSymbol = isErc20Reward
        ? erc20Symbol ||
          (isDaiToken ? "DAI" : isMetadataLoading ? "Loading..." : "ERC20")
        : farm.rewardCoin?.symbol || "???";
    const rewardDecimals = isErc20Reward
        ? erc20Decimals || (isDaiToken ? 18 : 18)
        : farm.rewardCoin?.decimals || 18;
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h4 className="font-mono font-bold text-primary">
                                JPYC {t("common.pool")} - {rewardSymbol}{" "}
                                {t("common.rewards_suffix")}
                            </h4>
                    ...
                    <div className="bg-muted/10 p-3 rounded border border-border">
                        ...
                        <div className="text-sm font-mono font-bold">
                            <span className="text-primary">
                                Base {baseApr.toFixed(1)}%
                            </span>
                            {farmApr > 0 && (
                                <span className="text-green-600">
                                    {" "}
                                    + Farm {farmApr.toFixed(1)}%
                                </span>
                            )}
                        </div>
                    </div>
                                        ...
                                        {isLoadingRewards ? (
                                            ...
                                            (() => {
                                                try {
                                                    ...
                                                    console.error(
                                                        "Error formatting pending rewards:",
                                                        error,
                                                    );
                                                    return "0";
                                                }
